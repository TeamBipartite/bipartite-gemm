...
{   
    ...
    constexpr int n =  utils::get_padded_sz(8192);
    ...
    *******************************
    */
    const std::vector<uint32_t> matrix_a = utils::generate_matrix( max_element, n*n );
    uint32_t* d_matrix_a;

    const std::vector<uint32_t> matrix_b = utils::generate_matrix( max_element, n*n );
    uint32_t* d_matrix_b;
    ...
    cudaMemset(d_matrix_c, 0x0, sizeof(uint32_t) * matrix_c.size() );
    ...
    */
    ...
    #else
        const std::vector<float> matrix_c_float( matrix_c.begin(), matrix_c.end() );
        const std::vector<float> matrix_c_expected = utils::cblas_cpu_matmul(matrix_a, matrix_b, n );
        utils::print_matrix<float>( matrix_c_expected, n, print_result );
        utils::print_matrix<float>( matrix_c_float, n, print_result );
        std::cout << "Correct Output:" << utils::matrices_equal<float>(matrix_c_float, matrix_c_expected) << std::endl;
    #endif
